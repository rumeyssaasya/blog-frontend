...
export function Header() {
  ...
  return (
    <header>
      ...
      style={{padding:'10px', backgroundColor: theme === 'dark' ? '#5d0ec0' : '#7008e7', transition:'background-color 0.3s'}}>
      ...
      <div style={{margin:'0 20px', borderRadius:'15px'}}>
        <nav className="flex justify-center gap-6 bg-indigo-100 dark:bg-violet-800 p-3 text-lg font-medium text-violet-900 dark:text-white"
          ...
          {links.map((link) => (
            ...
            <Search onResults={(results) => setPosts(results)} />
        </nav>
      </div>
      )}
    </header>
