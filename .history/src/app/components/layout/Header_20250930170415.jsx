  ...
  return (
    <header>
      ...
      <div className='flex items-center justify-center gap-6'>
        <nav className="md:flex items-center justify-center gap-6 bg-indigo-100 dark:bg-violet-800 p-3 text-lg font-medium text-violet-900 dark:text-white">
          {links.map((link) => (
            <Link key={link.href} href={link.href}>
              <button
                onClick={() => setActive(link.href)}
                className={`px-3 py-1 rounded transition-colors ml-4 flex-1 px-4 py-2${
                  active === link.href
