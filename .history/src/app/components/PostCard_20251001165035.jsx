...
export default function PostCard({ post }) {
  ...
  return (
    <div className="mb-6 max-w-150 h-20 flex justify-center items-center">
      {/* Resimli Post */}
      {post?.image ? (
        <motion.div
          className="relative flex flex-row justify-between rounded-2xl shadow-md transition-shadow duration-300 gap-1"
        ...
        // Resimsiz Post
        <motion.div
          className="relative flex flex-col justify-center items-center h-50 w-full rounded-2xl p-6 hover:shadow-xl shadow-md transition-shadow duration-300"
          whileHover={{ scale: 1.02, boxShadow: "0 8px 20px rgba(0,0,0,0.8)" }}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
