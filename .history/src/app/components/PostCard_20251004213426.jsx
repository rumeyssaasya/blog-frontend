...
function PostCard({ post, theme }) {
  ...
  const currentUserId = currentUser?._id || currentUser?.username;

  // Like state - backend verisine göre başlat
  const [likesCount, setLikesCount] = useState(Number(post.likesCount) || 0);
  const [liked, setLiked] = useState(false);

  // Like durumunu post verisine göre başlat
  useEffect(() => {
    setLikesCount(Number(post.likesCount) || 0);
    
    // Önce likedByUser kontrol et, yoksa likedByUsers array'ine bak
    if (post.likedByUser !== undefined) {
      setLiked(Boolean(post.likedByUser));
    } else if (Array.isArray(post.likedByUsers) && currentUserId) {
      setLiked(post.likedByUsers.includes(currentUserId));
    } else {
      setLiked(false);
    }
  }, [post.likesCount, post.likedByUser, post.likedByUsers, currentUserId]);
  ...
  const handleLike = async (e) => {
    ...
    try {
      ...
      console.log("Like Backend Response:", res);
      
      // Backend'den gelen güncel değerlerle state'i güncelle
      if (res.likesCount !== undefined) {
        setLikesCount(res.likesCount);
      }
      
      // likedByUser veya likedByUsers'a göre like durumunu güncelle
      if (res.likedByUser !== undefined) {
        setLiked(res.likedByUser);
      } else if (Array.isArray(res.likedByUsers)) {
        setLiked(res.likedByUsers.includes(currentUserId));
      }
  ...
  console.log("Post Like Debug:", {
    ...
    postLikedByUsers: post.likedByUsers,
    currentUserId,
    componentLikesCount: likesCount,
    componentLiked: liked
  });
        ...
        <div style={{
          ...
          >
            <TiHeart
              size={28}
              style={{
                color: liked ? "#ff4444" : (theme === "dark" ? "#e0e0e0" : "#18191a"),
