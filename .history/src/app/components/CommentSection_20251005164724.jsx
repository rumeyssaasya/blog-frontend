...
export default function CommentsSection({ postId }) {
  ...
  <div key={comment._id} style={{ marginBottom: "16px", borderBottom: "1px solid #ddd", paddingBottom: "10px" }}>
    <p>
      <strong>{comment.user?.username || comment.author?.username}:</strong> {comment.content} 
    </p>
    <div style={{ marginTop: "8px" }}>
      ...
      {/* Sadece yorum sahibi veya admin silme butonunu görebilir */}
      {user && (comment.user?._id === user._id || comment.author?._id === user._id) && (
        <button onClick={() => dispatch(deleteComment(comment._id))} style={{ color: "blue",cursor:'pointer' }}>
          Sil
        </button>
      )}
    </div>
      ...
      <div style={{ marginLeft: "20px", marginTop: "10px" }}>
        {comment.replies.map(reply => (
          <div key={reply._id} style={{ marginBottom: "6px" }}>
            <p>
              <strong>{reply.user?.username || reply.author?.username}:</strong> {reply.content} 
            </p>
            {user && (reply.user?._id === user._id || reply.author?._id === user._id) && ( // sadece reply sahibi görebilir
              <button
