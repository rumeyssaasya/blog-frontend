...
export default function MyProfile() {
  ...
  const handleAddComment = async () => {
    ...
    await dispatch(addComment({ postId: singlePost._id, content: commentText }));
    setCommentText("");
  };

  const handleAddReply = async (parentCommentId) => {
    if (!replyText.trim() || !currentUser) return alert("Yanıt eklemek için giriş yapmalısınız.");
    await dispatch(addReply({ commentId: parentCommentId, content: replyText }));
    setReplyText("");
    setReplyingTo(null);
    // Yanıtlar güncellendikten sonra yorumları yeniden yükle
                ...
                {comment.replies && comment.replies.length > 0 && (
                  <div style={{ marginLeft: "20px", marginTop: "10px" }}>
                    {comment.replies.map(reply => (
                      <div
                        key={`${comment._id}-${reply._id}`} // Benzersiz key
                        style={{
                          padding: "10px",
                          borderRadius: "6px",
