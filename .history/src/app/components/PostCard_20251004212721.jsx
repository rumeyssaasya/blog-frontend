...
function PostCard({ post, theme }) {
  ...
  // Like state'leri - backend verisine göre başlat
  const [likesCount, setLikesCount] = useState(Number(post.likesCount) || 0);
  
  // Kullanıcı bu postu beğenmiş mi? (likedByUsers array'inde currentUser var mı?)
  const [liked, setLiked] = useState(
    currentUser && Array.isArray(post.likedByUsers) 
      ? post.likedByUsers.includes(currentUser.username)
      : false
  );

  // Post veya currentUser değiştiğinde like durumunu güncelle
  useEffect(() => {
    if (post.likesCount !== undefined) {
      setLikesCount(Number(post.likesCount));
    }
    
    if (currentUser && Array.isArray(post.likedByUsers)) {
      setLiked(post.likedByUsers.includes(currentUser.username));
    }
  }, [post.likesCount, post.likedByUsers, currentUser]);
  ...
  const handleLike = async (e) => {
    ...
    try {
      // API'ye like isteği gönder
      const result = await dispatch(likePost(post._id)).unwrap();
      
      // Backend'den gelen güncel verileri kullan
      if (result.likesCount !== undefined) {
        setLikesCount(result.likesCount);
      }
      if (Array.isArray(result.likedByUsers)) {
        setLiked(result.likedByUsers.includes(currentUser.username));
      }
    } catch (error) {
