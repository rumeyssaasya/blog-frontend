    ...
    <div style={{ marginTop: "30px", backgroundColor: "#f9f9f9", borderRadius: "12px", padding: "16px" }}>
      ...
      {comments.map((comment) => (
        <div key={comment._id} style={{ marginBottom: "16px", borderBottom: "1px solid #ddd", paddingBottom: "10px" }}>
          <p>
            <strong>{user?.username}:</strong> {comment.content} {/* username giriş yapmış kullanıcı */}
          </p>
          <div style={{ marginTop: "8px" }}>
            ...
            {user && comment.user?._id === user._id && (
              <button onClick={() => dispatch(deleteComment(comment._id))} style={{ color: "crimson" }}>
                Sil
              </button>
            )}
          </div>
            ...
            <div style={{ marginLeft: "20px", marginTop: "10px" }}>
              {comment.replies.map(reply => (
                <div key={reply._id} style={{ marginBottom: "6px" }}>
                  <p>
                    <strong>{user?.username}:</strong> {reply.content} {/* reply username giriş yapmış kullanıcı */}
                  </p>
