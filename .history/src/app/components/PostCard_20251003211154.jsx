...
function PostCard({ post, theme, token, onDelete }) {
  ...
  function DynamicImage({ src, alt, width }) {
  ...
  useEffect(() => {
    ...
    img.src = src;
    img.onload = () => {
      setHeight((width / img.width) * img.height); // width sabit, height orantılı
    };
  }, [src, width]);
      ...
      >
        <div className="flex items-center gap-2">
          <MdOutlinePerson
            size={30}
            style={{
              color: theme === "dark" ? "#e0e7ff" : "#6b21a8",
            }}
          />
          <p className="text-l" style={{ marginTop: "10px" }}>
            {post.author.username}
          </p>
        </div>

        {/* Sadece token varsa üç nokta menüsü */}
        <ProtectedPage>
