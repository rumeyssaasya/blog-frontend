...
export default function MyProfile() {
  ...
  const [jwtDecode, setJwtDecode] = useState(null); // dinamik import
  ...
  useEffect(() => {
    import('jwt-decode').then(mod => setJwtDecode(() => mod));
  }, []);
  ...
  useEffect(() => {
    ...
    let userId = null;
    try {
      const decoded = jwtDecode(token); // artık direkt fonksiyon
      userId = decoded.id;
    } catch(err) {
  ...
  }, [jwtDecode, users, dispatch, status]);
