...
export const likePost = createAsyncThunk(
  ...
  async (postId, { rejectWithValue }) => {
    try {
      ...
      return response.data; // { likesCount: number, likedByUsers: string[] }
    } catch (error) {
      return rejectWithValue(error.response?.data?.message || 'Like işlemi başarısız');
    }
  }
);
...
const postSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    .addCase(likePost.fulfilled, (state, action) => {
      const index = state.items.findIndex(post => post._id === action.meta.arg);
      if (index !== -1) {
        state.items[index] = {
          ...state.items[index],
          likesCount: action.payload.likesCount,
          likedByUsers: action.payload.likedByUsers
        };
      }
      // Single post da güncellenmeli
      if (state.singlePost && state.singlePost._id === action.meta.arg) {
        state.singlePost = {
          ...state.singlePost,
          likesCount: action.payload.likesCount,
          likedByUsers: action.payload.likedByUsers
