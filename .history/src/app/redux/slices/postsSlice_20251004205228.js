...
export const likePost = createAsyncThunk(
  ...
  async (postId, { getState, rejectWithValue }) => {
    try {
      ...
      const { data } = await axios.post(`http://localhost:5000/api/posts/${postId}/like`, {}, config);
      return { postId, likesCount: data.likesCount, likedByUser: data.likedByUser };
    } catch (err) {
...
const postSlice = createSlice({
  ...
  extraReducers: builder => {
    ...
    .addCase(likePost.fulfilled, (state, action) => {
        ...
        if (post) {
          post.likes = Array(likesCount).fill(0); // sadece count için array
