...
export default function AdminPanel() {
  ...
  return (
    <ProtectedAdmin>
      ...
      {/* Tabs */}
      <div className="flex flex-row text-xl font-semibold justify-center gap-2 mb-6 bg-violet-500 text-white rounded-2xl cursor-pointer">
        {['users','posts','comments'].map(tab => (
          <button style={{cursor:"pointer"}} key={tab} onClick={() => setActiveTab(tab)} className={tabBtn(tab) }>
            {tab.toUpperCase()}
          </button>
        ))}
      </div>
      ...
      {activeTab === 'users' && (
        <section className={sectionClass}>
          <h2 className="text-xl font-semibold mt-2 mb-4 bg-violet-200">Kullanıcılar</h2>
          {/* Edit user form */}
          {editUser.id && (
            <div className="mb-4 p-3 bg-violet-100 dark:bg-violet-600 rounded">
          ...
          {users.map(user => (
            <div key={user._id} className="flex justify-between items-center bg-gray-100 dark:bg-violet-600 p-2 mb-2 rounded" style={{marginBottom:"1rem", marginTop:"1rem" , padding:"1rem"}}>
              <span>{user.username} ({user.email})</span>
              <div className="flex gap-2">
                <button onClick={() => setEditUser({ id:user._id, username:user.username, email:user.email })} className="text-blue-600 hover:text-blue-800 cursor-pointer">Düzenle</button>
                <button onClick={() => dispatch(deleteUserByAdmin(user._id))} className="text-red-600 hover:text-red-800 cursor-pointer">Sil</button>
              </div>
            </div>
          ))}
        </section>
