    ...
    <ProtectedAdmin>
      <div style={tabContainerStyle}>
        {['users','posts','comments'].map(tab => (
          <button key={tab} onClick={()=>setActiveTab(tab)} style={tabButtonStyle(activeTab===tab)}>{tab.toUpperCase()}</button>
        ))}
      </div>
          ...
          <section style={sectionStyle}>
            ...
            {editUser.id && (
              <div style={{display:'flex', gap:'12px', flexWrap:'wrap', marginBottom:'16px'}}>
                ...
                <button onClick={()=>{dispatch(updateUserByAdmin({id:editUser.id,data:{username:editUser.username,email:editUser.email}})); setEditUser({id:null,username:'',email:''})}} style={{...buttonStyle, backgroundColor:'#7C3AED'}}>Kaydet</button>
                <button onClick={()=>setEditUser({id:null,username:'',email:''})} style={{...buttonStyle, backgroundColor:'#9CA3AF'}}>İptal</button>
              </div>
            )}
            {users.map(user=>(
              <div key={user._id} style={cardStyle}>
                ...
                <div style={{display:'flex', gap:'12px'}}>
                  <button onClick={()=>setEditUser({id:user._id,username:user.username,email:user.email})} style={{...buttonStyle, backgroundColor:'#3B82F6'}}>Düzenle</button>
          ...
          <section style={sectionStyle}>
            ...
            {selectedPost && (
              <div style={{marginTop:'24px', padding:'16px', border:'1px solid #C4B5FD', borderRadius:'12px', backgroundColor:'#F3E8FF'}}>
                ...
                {postComments.length===0 ? <p>Bu post için yorum yok.</p> :
                ...
                {selectedComment && (
                  <div style={{marginTop:'16px', padding:'12px', border:'1px solid #C4B5FD', borderRadius:'12px', backgroundColor:'#EDE9FE'}}>
                    <h5 style={{fontWeight:'600'}}>Yorum Detayı</h5>
                    <textarea value={editComment.id===selectedComment._id ? editComment.content : selectedComment.content} 
                      onChange={e=>setEditComment({id:selectedComment._id,content:e.target.value})} 
                      style={{width:'100%', padding:'8px', marginBottom:'12px', borderRadius:'8px', border:'1px solid #C4B5FD'}}/>
                    <div style={{display:'flex', gap:'12px'}}>
            ...
            {comments.map(comment=>{
              ...
              return (
                <div key={comment._id} style={cardStyle}>
                  <span>{parentPost?.title || "Post silinmiş"}: {comment.author?.username || "Anonim"}: {comment.content}</span>
