        ...
        <div className="grid grid-rows w-fit justify-center text-center gap-6">
          {posts.map((post) => (
            ...
            >
              ...
              <p className="text-sm text-gray-500 mt-4">Yazar: {post.author}</p>
              <p className="text-sm text-gray-500">Oluşturulma Tarihi: {new Date(post.createdAt).toLocaleDateString()}</p>
            </div>
          ))}
        </div>
      )}
