...
export default function PostsPage() {
  ...
  return (
    <div className="container mx-auto p-4">
      ...
      {status === 'succeeded' && (
        <div className="grid grid-rows w-fit justify-center  gap-6">
          {posts.map((post) => (
            <div>
              <div className='flex items-center gap-2 mb-2'>
                <MdOutlinePerson size={30} className="text-violet-800 dark:text-indigo-100"/>
                <p className="text-m mt-4">Yazar: {post.author.username}</p>
              </div>
              <div
                key={post._id}
