...
import theme from '../components/layout/ThemeComp';
          ...
          {posts.map((post) => (
            <div className='' key={post._id}>
              <div className='flex items-center gap-2 mb-2'>
                <MdOutlinePerson size={20} style={{color: theme ==='dark' ? '#e0e7ff' : '#6b21a8',}}/>
                <p className="text-m mt-4">{post.author.username}</p>
              </div>
              
                {post?.image && (
                  <div key={post._id}
                    className="flex flex-row justify-between h-50 w-150   dark:bg-slate-800  rounded-lg p-6 hover:shadow-xl shadow-md transition-shadow duration-300 gap-4"
                    style={{color: theme ==='dark' ? 'slate-800' : 'white',}}
                  >
                ...
                {!post?.image && (
                  <div key={post._id}
                    className="flex justify-center items-center h-50 w-150 bg-white  dark:bg-slate-800  rounded-lg p-6 hover:shadow-xl shadow-md transition-shadow duration-300 gap-4"
                  >
