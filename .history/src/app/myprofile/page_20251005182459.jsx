...
export default function MyProfile() {
  ...
  useEffect(() => {
    if (authUser?._id && currentToken) {
      console.log("Fetching profile for user:", authUser._id);
      dispatch(getUserProfile({ userId: authUser._id, token: currentToken }));
    }
  }, [dispatch, authUser?._id, currentToken]);

  // Kullanıcının postlarını çek
  useEffect(() => {
    if (authUser?._id && currentToken) {
      console.log("Fetching posts for user:", authUser._id);
      dispatch(fetchUserPosts({ userId: authUser._id, token: currentToken }));
    }
  }, [dispatch, authUser?._id, currentToken]);
  ...
  }, [userProfile, authUser, posts, profileStatus]);
