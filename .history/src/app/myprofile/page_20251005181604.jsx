...
useEffect(() => {
  const fetchData = async () => {
    ...
    try {
      await dispatch(getUserProfile(authUser._id)).unwrap();
      const savedToken = token || localStorage.getItem("token");
      if (savedToken) {
        await dispatch(fetchUserPosts({ userId: authUser._id, token: savedToken })).unwrap();
      }
    } catch (err) {
      console.error("Profil veya post fetch hatası:", err);
    } finally {
      setLoading(false);
    }
  };
