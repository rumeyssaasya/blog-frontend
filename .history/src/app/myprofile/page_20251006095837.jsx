...
import { logout, hydrateUser } from "../redux/slices/authSlice";
...
export default function MyProfile() {
  ...
  useEffect(() => {
    if (authUser?._id) {
      ...
      dispatch(getUserProfile({ userId: authUser._id, token: effectiveToken }));
    }
  ...
  useEffect(() => {
    if (authUser?._id && (savedToken || token)) {
      const effectiveToken = savedToken || token;
      dispatch(fetchUserPosts({ userId: authUser._id, token: effectiveToken }));
    }
  }, [dispatch, authUser?._id, token, savedToken]);

  if (!userProfile) return <div className="text-center mt-10">Yükleniyor...</div>;
